...
import org.gnss.ignav.data.Odopt;
...
public final class InsOdo {
    ...
    private static int odoHVR(InsOpt opt, double[] Cbe, double[] lever, double[] Cbr, double[] ve,
        ...
        int ios = InsGnssState.xiOs(opt);
        int nos = InsGnssState.xnOs(opt);
        int iol = InsGnssState.xiOl(opt);
        int nol = InsGnssState.xnOl(opt);
        int ioa = InsGnssState.xiOa(opt);
        int noa = InsGnssState.xnOa(opt);
        int IV = InsGnssState.xiV(opt);
        int NV = InsGnssState.xnV(opt);
        int IA = InsGnssState.xiA(opt);
        int NA = InsGnssState.xnA(opt);
    ...
    private static void odoclp(double[] x, InsOpt opt, InsState ins) {
        ...
        if (noa > 0) InsMath.corratt(x, ins.Cbr);
